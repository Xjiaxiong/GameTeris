...
const { want, isClear, isOver } = unit


const getStartMatrix = (startLines) => { // 生成startLines
  const getLine = (min:number, max:number) => { // 返回标亮个数在min~max之间一行方块, (包含边界)
  ...
  let startMatrix:List<List<number>> = List([]);
...
type State = {
  ...
  clearLines: (matrix:List<List<number>>, lines:number[]) => void;
  dispatchPoints: (point:number) => void;
  focus: (isFocus:boolean) => void;
  nextAround: (matrix:List<List<number>>, stopDownTrigger?:() => void) => void;
  overEnd: () => void;
  overStart: () => void;
...
const states: State = {
  ...
  // 自动下落
  auto: (timeout) => {
    const out = ((timeout as number) < 0 ? 0 : timeout);
    ...
    clearTimeout(states.fallInterval as number);
